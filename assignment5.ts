 ...
 function findcharnotime(input_char:string,fnd_char:string): number{
   ...
   /* Question 32
    ...
    const toDoList: ToDolist_obj[] = [
        { task: "Meeting with OGRA", completed: false },
        { task: "Payroll run ", completed: true },
        { task: "Call to Vendor", completed: false },
        { task: "Submit Leave Application", completed: true }
    ];
...
/* Question 33
    ...
    function sortsmalltolarg(input_array:number[]):number[]{
        return input_array.sort((a, b) => a - b);
         }
      const tobesort_array = [2,1,5,3,6];
      sortsmalltolarg(tobesort_array);
      console.log(tobesort_array);
      
/* Question 34
Develop a TypeScript program that logs every element of 
an array in reverse order without using the .reverse() method.*/
    function reverse_array1(input_array:number[]){
    ...
    /* Question 35
    Write a script that simulates a basic calculator. It should 
    take two operands and an operator ('+', '-', '*', '/') 
...
console.log ("This is subtract");
 ...
 else { console.log("invalid operator. Please select valid operator from LOV.");
      }
